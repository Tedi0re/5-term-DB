...
begin
    ...
    dbms_output.put_line(faculty_rec.faculty || faculty_rec.faculty_name );
exception
    when no_data_found then dbms_output.put_line('данные не найдены (ORA' || sqlcode || ')');
    when others
    then dbms_output.put_line(sqlerrm);
end;
...
---------------------------------------7
begin
    insert into auditorium VALUES ('200-3b', '200-3b', 200, 'ЛК');
    commit;
    insert into auditorium VALUES ('200-3c', '200-3b', 200, 'ЛК');
    rollback;
exception
    when others
        then dbms_output.put_line(sqlcode || ' ' || sqlerrm);
